...
nclassifier.addDocument(NagativeWords.getData(), 'anegative');
nclassifier.train();
...
bgetSetiment:(comment)=>{

	let a=classifier.categorize(comment);
	let b=nclassifier.classify(comment)
...
ngetSetiment:(comment)=>{
	let a=analyzer.getSentiment(comment.split(" "));
